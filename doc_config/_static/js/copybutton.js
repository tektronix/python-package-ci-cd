...
function formatCopyText(
  ...
  const regexp = new RegExp(
    `^(${isRegexp ? copybuttonPromptText : escapeRegExp(copybuttonPromptText)})(.*)`
  );

  const outputLines = [];
  let isHereDocActive = false;

  for (const line of textContent.split("\n")) {
    const match = line.match(regexp);
    const isPromptLine = match !== null;

    if (isPromptLine || isHereDocActive || (useLineCont && line.endsWith(lineContinuationChar))) {
      if (removePrompts && isPromptLine) {
        outputLines.push(match[2]);
      } else {
        outputLines.push(line);
      }
      isHereDocActive = useHereDoc && line.includes(hereDocDelim) ? !isHereDocActive : isHereDocActive;
    } else if (!onlyCopyPromptLines || (copyEmptyLines && line.trim() === "")) {
      outputLines.push(line);
    }
  }

  // Remove trailing newline to avoid auto-running when pasting
